$(document).ready(function() {
    ...
    //Business locations CRUD
    warehouse_locations = $('#warehouse_location_table').DataTable({
        processing: true,
        serverSide: true,
        bPaginate: false,
        fixedHeader:false,
        buttons: [],
        ajax: '/warehouse-location',
        columnDefs: [
            {
                targets: 11,
                orderable: false,
    ...
    $('.location_add_modal, .location_edit_modal').on('shown.bs.modal', function(e) {
        $('form#warehouse_location_add_form')
            .submit(function(e) {
                e.preventDefault();
            })
            .validate({
                rules: {
                    location_id: {
                        remote: {
                            url: '/warehouse-location/check-location-id',
                            type: 'post',
                            data: {
                                location_id: function() {
                                    return $('#location_id').val();
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            if (result.success == true) {
                                ...
                                $('div.location_edit_modal').modal('hide');
                                toastr.success(result.msg);
                                warehouse_locations.ajax.reload();
                            } else {
                                toastr.error(result.msg);
                            }
                        },
        ...
        $('form#warehouse_location_add_form').find('#featured_products').select2({
            minimumInputLength: 2,
            allowClear: true,
            placeholder: '',
            ajax: {
