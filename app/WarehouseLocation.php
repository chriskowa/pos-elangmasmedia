...
class WarehouseLocation extends Model
{
    ...
    {
        $query = WarehouseLocation::where('business_id', $business_id)->Active();
        ...
        if ($append_id) {
            $query->select(
                DB::raw("IF(warehouse_id IS NULL OR warehouse_id='', name, CONCAT(name, ' (', warehouse_id, ')')) AS name"),
                'id',
    ...
    public function getFeaturedProducts($is_array = false, $check_location = true)
    {
        ...
        if ($check_location) {
            $query->where('pl.warehouse_id', $this->id);
        }
