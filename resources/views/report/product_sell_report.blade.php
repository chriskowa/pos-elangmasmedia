        ...
        <div class="col-md-12">
            @component('components.filters', ['title' => __('report.filters')])
              {!! Form::open(['url' => action([\App\Http\Controllers\ReportController::class, 'getStockReport']), 'method' => 'get', 'id' => 'product_sell_report_form' ]) !!}
                ...
                <div class="col-md-3">
                <div class="form-group">
                            {!! Form::label('variation_new_id', __('Bisnis Lokasi') . ':') !!}
                            {!! Form::select('variation_new_id', $variation_values, null, [
        ...
        <div class="col-md-12">
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs">
                    ...
                    <li>
                        <a href="#psr_by_variation_tab" data-toggle="tab" aria-expanded="true"><i class="fa fa-bars" aria-hidden="true"></i> By Bisnis Lokasi</a>
                    </li>
                </ul>
                <div class="tab-content">
                    ...
                    @include('report.partials.product_sell_report_by_variation')
                </div>
            </div>
        </div>
    </div>
...
@section('javascript')
    ...
    $(
        '#product_sell_report_form #location_id, #product_sell_report_form  #variation_new_id, #product_sell_report_form #customer_id, #psr_filter_brand_id, #psr_filter_category_id, #psr_customer_group_id'
    ).change(function() {
    ...
    $(document).ready(function() {
        $('a[data-toggle="tab"]').on('shown.bs.tab', function(e) {
            ...
            } else if (target == '#psr_by_variation_tab') {
                if (typeof product_sell_report_by_variation_datatable == 'undefined') {
                    product_sell_report_by_variation_datatable = $('table#product_sell_report_by_variation').DataTable({
                        processing: true,
                        serverSide: true,
                        fixedHeader: false,
                        ajax: {
                        ...
                        fnDrawCallback: function(oSettings) {
                            $('#footer_psr_by_variation_total_sell').text(
                                sum_table_col($('#product_sell_report_by_variation'), 'row_subtotal')
                            );
                            $('#footer_psr_by_variation_total_sold').html(
                                __sum_stock($('#product_sell_report_by_variation'), 'sell_qty')
                            );

                            $('#footer_psr_by_variation_total_stock').html(
                                __sum_stock($('#product_sell_report_by_variation'), 'current_stock')
                            );
                            __currency_convert_recursively($('#product_sell_report_by_variation'));
                        },
                    });
                } else {
                    product_sell_report_by_variation_datatable.ajax.reload();
                }
            }
        });
    });
